...
function Get-TargetData-Local {
    <#
    .SYNOPSIS
    Runs each module against on the localhost as the current user. Writes out the returned data to host where Kansa is run from.
    #>
	Param(
	    [Parameter(Mandatory=$True,Position=0)]
    ...
    $Modules.Keys | Foreach-Object { $Module = $_
        ...
        foreach($bindep in $bindeps) {
            ...
            if ($bindep) {
                ...
                $RemoteWindir = Invoke-Command -ScriptBlock { Get-ChildItem -Force env: | Where-Object { $_.Name -match "windir" } | Select-Object -ExpandProperty value }
                $RemoteWindir
                $null = Send-File -Path (ls $bindep).FullName -Destination $RemoteWindir
        ...
        Write-Debug "Invoke-Command -FilePath $Module -AsJob -ThrottleLimit $ThrottleLimit"

        $sb = Get-Command $Module | select -ExpandProperty ScriptBlock
        if ($Arguments) {
            $job = Invoke-Command -ScriptBlock $sb -ArgumentList $Arguments
        } else {
            $job = Invoke-Command -ScriptBlock $sb
        }
		...
		$Outfile = $OutputPath + $GetlessMod + $ArgFileName + "\" + $env:COMPUTERNAME + "-" + $GetlessMod + $ArgFileName
...
if ($Outputfolder) { 
	$OutputPath = $StartingPath + "\" + $Outputfolder + "\"
} else {
...
$Targets
$Targets.count
if ($Targets.count -eq 1 -and ($Targets[0] -eq "localhost" -or $Targets[0] -eq $env:COMPUTERNAME)) {
    Get-TargetData-Local -Modules $Modules -ThrottleLimit $ThrottleLimit
} elseif ($OutputFormat -eq "csv" -or $OutputFormat -eq "json" -or $OutputFormat -eq "tsv" -or $OutputFormat -eq "xml") {
